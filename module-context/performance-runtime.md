...
## Current behavior after v20260501_1921_fast_cool_runtime
...
- `callApi()` can write GET snapshots only when callers pass `cacheTtlMs`; it does not cache every GET by default to avoid sessionStorage quota/jank.
- `renderApiSnapshot()` shows cached data immediately before network refresh for Dashboard, Recent Purchases, Stock summary/search, Supplier debt, Customer debt, Supplier returns.
...
- `/1` contains current `window.__TK_APP_VERSION__`, `API_SNAPSHOT_PREFIX`, `APP_UPDATE_CHECK_MS = 60000`, and rollout `enabled: false`.
