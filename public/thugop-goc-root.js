...
} from "./legacy-core/thugop-goc-repo.js";
...
if (!host) {
  ...
  async function fetchCurrentActorApproveToken() {
    const data = await callInternalPosApi("/internal/pos/thugop-goc/handover-token", {
      method: "GET",
    });
  ...
  async function verifyCurrentActorApproveToken(token = "") {
    return callInternalPosApi("/internal/pos/thugop-goc/handover-token/verify", {
      method: "POST",
  ...
  async function reserveThuGopCreateCode(force = false) {
    ...
    state.createReservedCodePromise = (async () => {
      const data = await callInternalPosApi("/internal/pos/thugop-goc/reserve-code", {
        method: "POST",
        body: {},
      });
  ...
  function publishThuGopHeaderSummary() {
    ...
    window.__THKD_THUGOP_GOC_HEADER_SUMMARY__ = detail;
    try {
